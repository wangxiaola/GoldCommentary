...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        if(self.cityDataSouce.count == 0){
            ...
            [self setUpAddress:provinceItem.name cityCode:provinceItem.code];
            return indexPath;
        }
        ...
        [self setUpAddress:item.name cityCode:item.code];
    }
    return indexPath;
}
...
- (void)setUpAddress:(NSString *)address cityCode:(NSString*)code{
