...
+ (NSInteger)getWeek_AccordingToYear:(NSInteger)year {
    ...
    NSDateComponents *comps = [WCDataTimeTool getDateComponents:[WCDataTimeTool dateFromString:[NSString stringWithFormat:@"%ld-12-31",year] DateFormat:@"yyyy-MM-dd"]];
...
+ (NSString*)getWeekRangeDate_Year:(NSInteger)year WeakOfYear:(NSInteger)weekofYear
{
    ...
    if ([WCDataTimeTool getWeek_AccordingToYear:year] == 53) {
        todayIsWeek += 1;
    }
    ...
    weekDate = [NSString stringWithFormat:@"第%ld周(%@-%@)",weekofYear,[WCDataTimeTool stringFromDate:firstDayOfWeek DateFormat:@"yyyy年M月d号"],[WCDataTimeTool stringFromDate:lastDayOfWeek DateFormat:@"yyyy年M月d号"]];
...
+ (NSDateComponents *)getCurrentDateComponents {
    return [WCDataTimeTool getDateComponents:[NSDate date]];
}
+ (NSInteger)getCurrentWeek {
    NSInteger week = [[WCDataTimeTool stringFromDate:[NSDate date] DateFormat:@"w"] intValue];
    return week;
}
+ (NSInteger)getCurrentYear{
    NSInteger year = [[WCDataTimeTool stringFromDate:[NSDate date] DateFormat:@"y"] intValue];
    return year;
}

+ (NSInteger)getCurrentQuarter{
    NSInteger quarter = [[WCDataTimeTool stringFromDate:[NSDate date] DateFormat:@"q"] intValue];
    return quarter;
}

+ (NSInteger)getCurrentMonth{
    NSInteger month = [[WCDataTimeTool stringFromDate:[NSDate date] DateFormat:@"M"] intValue];
    return month;
}

+ (NSInteger)getCurrentDay{
    NSInteger day = [[WCDataTimeTool stringFromDate:[NSDate date] DateFormat:@"d"] intValue];
    return day;
}
...
+ (BOOL)compareStartDay:(NSString *)startDay withendDay:(NSString *)endDay{
    ...
    {
        //NSLog(@"dateA比 dateB时间晚");
        return YES;
        
    } else if (result == NSOrderedAscending){
       //NSLog(@"dateA比 dateB时间早");
        return NO;
...
+ (NSArray *)getMonthFirstAndLastDayWith:(NSString *)dateStr{
    ...
    [myDateFormatter setDateFormat:@"yyyy-MM-dd"]; NSString *firstString =
    [myDateFormatter stringFromDate: firstDate]; NSString *lastString =
    [myDateFormatter stringFromDate: lastDate];
