...
- (void)replaceObjectAtIndex:(NSUInteger)index withString:(NSString *)anStr;
{
    if (index >= self.tagsMutableArray.count) {
        return;
    }
    [self.tagsMutableArray replaceObjectAtIndex:index withObject:anStr];
     FMTagModel *tagModel = [[FMTagModel alloc] initWithName:anStr font:self.tagFont];
    [self.tagModels replaceObjectAtIndex:index withObject:tagModel];
    [self.collectionView reloadData];
    [self invalidateIntrinsicContentSize];
}
